...
/*comeca aqui paginacao*/
	...
	//Verificar se está sendo passado na URL a página atual, se nao é atribuido a pagina 
		$pagina = (isset($_GET['pagina']))? $_GET['pagina'] : 1;
			if(!isset($_GET['pesquisar3'])){
				header("Location: ../index.html");
			}else{
				$valor_pesquisar = $_GET['pesquisar3'];
			}
		$maxlinks = 4;
	//Selecionar todos os clientes da tabela
		$result_cliente = "SELECT * FROM cliente WHERE UF LIKE '%$valor_pesquisar%'";
	...
	//Selecionar os clientes a serem apresentado na página
		$comandoSQL = "SELECT * FROM cliente WHERE UF LIKE '%$valor_pesquisar%' ORDER BY nome_cliente limit $inicio, $quantidade_pg";
...
/*-----------------------------------------------------------------------------------------*/
	...
	while($contador<=$contarLinhas){
		$dados = mysqli_fetch_array($executar);
			...
			echo"<tr class='butn1'>";
			echo"<td>";
			echo"&nbsp <a href='resultadoUF.php?cod=$codigo'><b>$nome</b></a><br>";
			echo"</td>";	
			echo"<td>";
			echo"&nbsp <a href='resultadoUF.php?cod=$codigo'>  $razaoSocial </a>" ;
			echo"</td>";
			echo"<td>";
			echo"&nbsp  $uf" ;
			echo"</td>";
	...
	<?php
		...
		<div id="botao_paginacao">
				<?php if($pagina_anterior != 0){ ?>
					<a href="pesquisar3.php?pagina=1&pesquisar3=<?php echo $valor_pesquisar; ?>"  aria-label="Previous" class="botao_paginacao">
				...
				<?php if($pagina_anterior != 0){ ?>
					<a href="pesquisar3.php?pagina=<?php echo $pagina_anterior; ?>&pesquisar3=<?php echo $valor_pesquisar; ?>" aria-label="Previous" class="botao_paginacao">
				...
				<?php 
					...
					if($i>=1){?>
					<a href="pesquisar3.php?pagina=<?php echo $i; ?>&pesquisar3=<?php echo $valor_pesquisar; ?>" class="botao_paginacao"><?php echo $i; ?></a>
					<?php } }
					//-----------------------------
				...
				<?php
					...
					<a href="pesquisar3.php?pagina=<?php echo $pagina_posterior; ?>&pesquisar3=<?php echo $valor_pesquisar; ?>" aria-label="Previous" class="botao_paginacao">
				...
				<?php
					if($pagina_posterior <= $num_pagina){ ?>
						<a href="pesquisar3.php?pagina=<?php echo $num_pagina; ?>&pesquisar3=<?php echo $valor_pesquisar; ?>" aria-label="Previous" class="botao_paginacao">
