<html>
	...
	<body id ="bodyLista">
		...
		<?php
		if ( !isset($_GET["cod"]))
					die("<i>Erro ao tentar buscar resgistro</i> <br>"."<div id='botoes'><a href='lista.php'><input type='button' id ='voltar' value='Voltar' class='butn'></a></div>");
		...
		//CONECTANDO NO SERVIDOR MYSQL
			...
			mysqli_select_db($conexao,$database) or 
				die ("<i>Erro na seleção do banco </i>" . mysqli_error ($conexao)."<div id='botoes'><a href='clientes.html'><input type='button' id ='voltar' class='butn' value='Voltar'></a></div>");
			...
			$executar = mysqli_query($conexao,$comandoSQL) or 
				die("<i>Falha na seleção de dados </i>" . mysqli_error($conexao)."<div id='botoes'><a href='clientes.html'><input type='button' id ='voltar' class='butn' value='Voltar'></a></div>");
			
			$contarLinhas = mysqli_num_rows($executar) or 
				die ("<h4><i>Não foram encontrados registros</i>" . mysqli_error($conexao)."</h4><br>"."<div id='botoes'><a href='clientes.html'><input type='button' id ='voltar' class='butn' value='Voltar'></a></div>");
			if ($contarLinhas==0)
				die("<i>Não foram encontardo(s) resgistro(s)</i>"."<div id='botoes'><a href='clientes.html'><input type='button' id ='voltar'  class='butn' value='Voltar'></a></div>");
